...
async function createStudent(data : {firstName: string; 
    ...
    p1_surname:string; 
     p1_address1: string; 
     p1_city: string; p1_province: string; p1_postalCode: string; p1_dateOfBirth: string; 
     p1_gender: string; p1_idNumber: string; p1_occupation: string; p1_phoneNumber: string; 
    ...
    async function fetchStudent( ) {
            ...
            try {
                const response = await database.listDocuments(
                    process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID as string,
                    "students", [Query.orderDesc("$createdAt")]
                );
...
export async function POST(request: Request) {
    try {
        const {firstName, secondName, surname, address, city, 
            ...
            p1_surname, p1_address1, p1_city, p1_province, p1_postalCode, p1_dateOfBirth, 
