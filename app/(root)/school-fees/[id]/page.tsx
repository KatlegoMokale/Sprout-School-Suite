...
const StudentInvoice = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setTransactions(transactionsData.filter(t => t.studentId === unwrappedParams.id));
        setStudentFees(studentFeesData.filter(f => f.studentId === unwrappedParams.id));
        setSchoolFees(schoolFeesData);

        const items = generateStatementItems(
          transactionsData.filter(t => t.studentId === unwrappedParams.id),
          studentFeesData.filter(f => f.studentId === unwrappedParams.id),
          schoolFeesData
        );
